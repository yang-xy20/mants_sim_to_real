...
class ResNetEncoder(nn.Module):
    def __init__(
        ...
        super().__init__()
        # print(observation_space["panoramic_depth"])
        if isinstance(observation_space, int):
            self._n_input_rgb = 3
            self._n_input_depth = 1 if observation_space > 3 else 0
            spatial_size = 64
        else:
            if "graph_panoramic_rgb" in observation_space:
                self._n_input_rgb = observation_space["graph_panoramic_rgb"].shape[3]
                spatial_size = observation_space["graph_panoramic_rgb"].shape[0] // 2
            else:
                self._n_input_rgb = 0

            if "graph_panoramic_depth" in observation_space:
                self._n_input_depth = observation_space["graph_panoramic_depth"].shape[3]
                spatial_size = observation_space["graph_panoramic_depth"].shape[0] // 2
            else:
