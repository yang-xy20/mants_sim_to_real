...
class MIXBase(nn.Module):
    def __init__(self, args, obs_shape, action_space, cnn_layers_params=None, cnn_last_linear = True, graph_linear = True):
        ...
        self.use_map_critic = False
        self.use_batch_train = False
        self.cnn_keys = []
        self.embed_keys = []
        self.mlp_keys = []
        self.local_cnn_keys = []
        ...
        if self._agent_invariant and self.invariant_type not in ["split_attn", "alter_attn"]:
        ...
        if len(self.graph_keys) > 0:#self.hidden_size = 512, num_recurrent_layers = 2, rnn_type="LSTM", backbone="resnet18", normalize_visual_inputs=True 
            self.graph = self._build_graph_model(obs_shape, action_space, args)
        
        if not self.build_graph or (self.use_map_critic and self.graph_linear):
            if self._agent_invariant and self.invariant_type in ["split_attn", "alter_attn"]:
