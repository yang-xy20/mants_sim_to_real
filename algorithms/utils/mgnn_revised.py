...
class Perception_Graph(torch.nn.Module):
    def __init__(self, args, graph_linear):
        ...
        self.gnn = AttentionalGNN(feature_dim, gnn_layers, False, use_double_matching=self.use_double_matching,matching_type=self.matching_type)
        if self.use_double_matching:
            self.node_gnn = AttentionalGNN(feature_dim, node_gnn_layers, False, node_layer=True)
        self.cuda = args.cuda
        self.use_double_matching = args.use_double_matching
