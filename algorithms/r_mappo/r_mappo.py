...
class R_MAPPO():
    def __init__(self,
        ...
        self.use_scene_norm = False#args.use_scene_norm
    ...
    def cal_value_loss(self, value_normalizer, values, value_preds_batch, return_batch, active_masks_batch, rank_batch):
        value_pred_clipped = value_preds_batch + (values - value_preds_batch).clamp(-self.clip_param, self.clip_param)
        
        if self._use_popart:
            error_clipped = value_normalizer.normalize(return_batch, rank_batch) - value_pred_clipped
            error_original = value_normalizer.normalize(return_batch, rank_batch) - values
        elif self._use_valuenorm:
    ...
    def ppo_update(self, sample, turn_on=True):
        ...
        rank_batch = check(rank_batch).to(**self.tpdv).long()
        ...
        values, action_log_probs, dist_entropy, policy_values = self.policy.evaluate_actions(share_obs_batch,
                                                                              ...
                                                                              available_actions_first_batch, 
                                                                              available_actions_second_batch,
                                                                              active_masks_batch,
                                                                              rank=rank_batch)
        # actor update
        ...
        if self._use_policy_vhead:
            policy_value_loss = self.cal_value_loss(self.policy_value_normalizer, policy_values, value_preds_batch, return_batch, active_masks_batch, rank_batch)       
        ...
        # critic update
        value_loss = self.cal_value_loss(self.value_normalizer, values, value_preds_batch, return_batch, active_masks_batch, rank_batch)
    ...
    def train(self, buffer, turn_on=True):
        if self._use_popart:
            advantages = self.value_normalizer.normalize(buffer.returns[:-1], buffer.rank[:-1]).cpu().numpy() - buffer.value_preds[:-1]
        elif self._use_valuenorm:
        ...
        if self.use_scene_norm:
            ...
            mean_advantages = np.nanmean(advantages_copy, axis=(0,2,3), keepdims=True)
            std_advantages = np.nanstd(advantages_copy, axis=(0,2,3), keepdims=True)
            advantages = (advantages - mean_advantages) / (std_advantages + 1e-5)
        elif not self._use_popart:
            advantages_copy = advantages.copy()
            advantages_copy[buffer.active_masks[:-1] == 0.0] = np.nan
        ...
        train_info['ratio'] = 0
        train_info['std_advantages'] = 0
        train_info['mean_advantages'] = 0
        for _ in range(self.ppo_epoch):
            ...
            for sample in data_generator:
                ...
                train_info['std_advantages'] += std_advantages.item()
                train_info['mean_advantages'] += mean_advantages.item()
        ...
        if self._use_popart:
            buffer.update_normalizer(self.value_normalizer)
